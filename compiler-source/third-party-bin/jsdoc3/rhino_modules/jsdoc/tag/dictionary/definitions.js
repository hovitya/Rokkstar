...
exports.defineTags = function(dictionary) {
    ...
    //
    // ROKKSTAR SPECIFIC DEFINITIONS
    //
    dictionary.defineTag('package', {
        mustHaveValue: true,
        onTagged: function(doclet, tag) {
            doclet.package=tag.value;
        }
    });

    dictionary.defineTag('argument', {
        mustHaveType: true,
        canHaveName: true,
        onTagged: function(doclet, tag) {
            if (!doclet.arguments) { doclet.arguments = []; }
            doclet.arguments.push(tag.value);
        }
    })
    .synonym('arg');
